...
%                           data directories
%                  runwc - char array (or 2-row char matrix) that defines a
    ...
    if ischar(varargin{i})
        switch varargin{i}
            ...
            case 'no_reorientation'
                do_reorient = 0;
            otherwise
                warning('1','Unknown character varargin entered.  Unknown use, skipping')
        end
    end
end
...
if do_part1
    runcommandstring = 'canlab_preproc_2012(subjectdir, runwc, imgwc, TR, anat, ''acquisition'', acqorder, ''nointeractive'',''nopart2''); \n';
    jobstr = 'preproc_part1';
    fprintf('Initiating preproc_part1: Slice acquisition timing and motion correction\n\n')
    for subj = subjs_to_run
        runcommand(subj,EXPT,scriptdir,jobstr,datadir,basedir,runcommandstring,TR,anat,acqorder,runwc,imgwc);
    end
...
function runcommand(subj,EXPT,scriptdir,jobstr,datadir,basedir,runcommandstring,TR,anat,acqorder,runwc,imgwc) %#ok
